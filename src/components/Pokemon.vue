<template>
  ...
  >
    ...
    <v-card-subtitle>Poids: {{ pokemon.weight }}</v-card-subtitle>
    ...
    <v-card-actions>
      ...
      >
      <v-spacer></v-spacer>
      <v-btn v-if="caught?false:true" class="bg-red-accent-4" @click="$emit('catched', props.pokemonID);caught=true"> Attraper </v-btn>
    </v-card-actions>
  </v-card>
  <v-skeleton-loader
    v-else
